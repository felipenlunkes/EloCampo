...
export default function PerfilEmpresa() {
  ...
  const inp: React.CSSProperties = { width:'100%',height:32,background:'#0a1520',border:'0.5px solid #1a3050',borderRadius:7,padding:'0 10px',fontSize:12,color:'#70aadd',outline:'none' }
  const lbl: React.CSSProperties = { fontSize:10,fontWeight:600,color:'#4a7090',textTransform:'uppercase',letterSpacing:'0.06em',display:'block',marginBottom:4 }
        ...
        <div className="topbar" style={{ background:'var(--buyer-bg)', borderBottomColor:'var(--buyer-border)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ position:'relative', flexShrink:0 }}>
              {fotoPerfil
                ? <img src={fotoPerfil.secureUrl} alt="" style={{ width:64, height:64, borderRadius:'50%', objectFit:'cover', border:'2px solid #1a3050' }} />
                : <div style={{ width:64, height:64, borderRadius:'50%', background:'#0a1520', border:'2px solid #1a3050', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, color:'#4a7090' }}>
              ...
              <button
                ...
                title="Alterar foto"
                style={{ position:'absolute', bottom:0, right:0, width:20, height:20, borderRadius:'50%', background:'#0d2035', border:'1px solid #1a3050', color:'#4a7090', fontSize:10, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:0 }}
            ...
            <div>
              <h2 className="topbar-title">Perfil da Empresa</h2>
              <p className="topbar-sub" style={{ color:'#4a7090' }}>{session?.account?.name ?? session?.email}</p>
        ...
        <div className="page-body">
          ...
          <form onSubmit={handleDados}>
            <div className="card buyer">
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:10 }}>
                <div style={fg}><label style={lbl}>Nome / Responsável</label><input style={inp} value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} required /></div>
              ...
              <div style={{ fontSize:10, fontWeight:600, color:'#4a7090', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:8, marginTop:4 }}>Endereço</div>
              ...
              <button type="submit" className="btn btn-buyer" disabled={loading}>{loading?'Salvando...':'Salvar alterações'}</button>
            </div>
          </form>
          ...
          <form onSubmit={handleSenha}>
            <div className="card buyer">
              ...
              <button type="submit" className="btn btn-buyer">Atualizar senha</button>
            </div>
          </form>
