...
O `account-service` é a aplicação responsável pelo cadastro de contas na plataforma.
...
* `account-service-contract`: aqui, temos classes comuns, que serão utilziadas dentro da implementação, mas que também podem
  ser exportadas e importadas em outros projetos que queiram utilizar a API disponibilizada pelo `account-service`. Fazemos isso
  para poder versionalizar o contrato e permitir reuso;
* `account-service-impl`: neste pacote, está a implementação de fato da aplicação, assim como seus testes unitários e de integração.

### O pacote de contrato

O pacote de contrato (`account-service-contract`) é subdivido em domínios, e tem código utilizado dentro e fora da aplicação,
permitindo requisições REST.

### O pacote de implementação

Neste pacote temos a implementação de fato, também dividida em seus respectivo domínio, `account`.
...
* `account`: domínio de conta, com toda a lógica de negócio e persistência segregada;
    - `controller`: classe que define os endpoints relacionados à conta, com os mapeamentos dentro da semântica HTTP;
...
Na task `AccountServiceApplication` do Gradle, adicione no campo `Run`,o argumento a seguir:
...
```shell
./gradlew :account-service-contract:publishToMavenLocal
``` 